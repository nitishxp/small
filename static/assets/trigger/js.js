...
<script>
    ...
    $(document).on('change', '.query', function (e) {
        ...
        if (inputQuery['data'].length > 0) {
            ...
            $.ajax({
                method: 'POST',
                data: { 'csrfmiddlewaretoken': '{{csrf_token}}', 'inputQuery': JSON.stringify(inputQuery) },
    ...
    $("[name='submit']").on('click', function () {
        ...
        query = {
            'csrfmiddlewaretoken': '{{csrf_token}}', 'campaign_name': campaign_name,
    ...
    $(".addfilter").on('click', function () {
        ...
        var f = '<form action="." method="post"><div class="row"><div class="col col-lg-6 col-lg-offset-5"><select class="panel form-group query" name="joinCondition"><option value="and">And</option><option value="or">Or</option></select></div></div><div class="row"> <div class="col col-lg-10"> <div class="col-md-3"> <div class="form-group"> <h6>Filter</h6> <select class="col-lg-12 query filter" name="filter" data-target_name="filter_level"> <option value="">Select A Value</option> <option value="predictions">Predictions</option> <option value="profile">Profile</option> </select> </div> </div> <div class="col-md-3"> <div class="form-group"> <h6>Choose Type</h6> <select class="col-lg-12 query" name="type"> <option value="<">Less Than</option> <option value="<=">Less Than Equal To</option> <option value="=">Equal To</option> <option value=">=">Greater Than Equal To</option> <option value=">">Greater Than</option> </select> </div> </div> <div class="col-md-3"> <div class="form-group"> <h6>Filter Level</h6> <select class="col-lg-12 query" name="filter_level"> </select> </div> </div> <div class="col-md-3"> <div class="form-group"> <h6>Value</h6> <input type="text" class="query trigger_input" name="value"> </div> </div> </div> <div class="col col-lg-2 text-center"><button id="removefilter" type="button" onclick="removered()" class="btn btn-default btn-sm trigger_remove panel"><span class="glyphicon glyphicon-remove"></span> REMOVE </button></div> </div> </form>'

        $(".addmore").append(f)
    })
    $("#resetbtn, #submit").on("click", function () {
        $('#showdemo form').each(function () {
            $(this)[0].reset();
        })
        $('[name=submit]').prop('disabled', true);
        $('[name=platform]').prop('disabled', true);
    ...
    $(document).on('click', '.deletebtn', function () {
        ...
        if (askConfirm) {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: { 'csrfmiddlewaretoken': '{{csrf_token}}', 'unique_campaign_id': unique_campaign_id },
                success: function (result) {
                    if (result == true) {
