...
$("#add_assignments").on('click', function () {
    ...
    //template += '</td>'
    template += '<td>'
    template += '<label>No. of Students Per Group </label><br>'
    template += '<input type="number" min="1" value="3" name="' + currentAssignmentLength + '___no_of_group"  class="form-control" required>'
    template += '</td>'
    template += '<td>'
    template += '<label>No. of Graders</label><br>'
    template += '<select name="' + currentAssignmentLength + '___no_of_grader" class="form-control" required>'
